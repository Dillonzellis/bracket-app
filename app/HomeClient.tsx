...
import EntrantsSection from "./EntrantsSection";
...
export default function HomeClient({
  ...
  const isAdmin = !loggedOut && (!!user || debugMode || serverIsAdmin);
  ...
  const loadStub = (count: number) => {
    ...
    setSeeds(
      Object.fromEntries(Array.from({ length: count }, (_, i) => [i, i + 1])),
    );
  };
  ...
  return (
    <main className="min-h-screen flex flex-col items-center py-12 px-4">
      <div className="text-center mb-10">
        <div className="text-xs md:text-sm mb-1 text-(--text-dim)">
        ...
        <div className="mt-2">
          {user ? (
            <button
              onClick={logout}
              className="text-xs font-mono text-(--text-dim) hover:text-[#e8001c] transition-colors"
            >
              logout ({user.email})
            </button>
          ) : debugMode ? (
            ...
            <button
              ...
              className="text-xs font-mono text-(--text-dim) hover:text-[#39ff14] transition-colors"
            >
              admin login
            </button>
          )}
        </div>
        <div className="text-xs md:text-sm tracking-widest mt-1 text-(--text-dim)">
      ...
      {loadingTournaments ? (
        <div className="w-full max-w-md mb-8">
          <div className="text-sm mb-2 text-(--text-dim)">&gt; TOURNAMENTS</div>
          <div className="text-sm font-mono text-(--text-dim) animate-pulse">
            loading...
          </div>
        </div>
      ) : (
            ...
            <div className="space-y-1">
              {tournaments.map((t) => (
                ...
                >
                  ...
                  {(() => {
                    const status = t.state.champion
                      ? "complete"
                      : Object.values(t.state.matches).some((m) => m.winner)
                        ? "in_progress"
                        : "not_started";
                    const cfg = {
                  ...
                  {isAdmin && (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="text-sm text-(--text-dim) hover:text-[#e8001c] transition-colors font-mono px-1 cursor-pointer"
                      >
                        ✕
                      </button>
                    </div>
      ...
      {isAdmin ? (
        <>
          ...
          <div ref={formRef} className="w-full max-w-md mb-5">
            ...
            <div className="flex gap-2 items-center">
              <span className="text-sm text-(--text-dim)">FORMAT:</span>
              {([3, 5] as const).map((f) => (
                <button
                  key={f}
            ...
            <div className="flex gap-1.5 flex-wrap">
              {[3, 4, 5, 6, 7, 8, 10, 12, 16, 24, 32].map((n) => (
                <button
                  key={n}
                  onClick={() => loadStub(n)}
                  className="text-sm px-2 py-1 font-mono border border-(--border) text-(--text-dim) bg-transparent hover:border-(--text) hover:text-(--text) transition-colors"
                >
                  {n}P
                </button>
              ))}
          ...
          <div className="w-full max-w-md space-y-2">
            ...
            {names.map((name, i) => (
              <div key={i} className="flex gap-2 items-center">
                <span className="text-sm w-5 text-right text-(--text-dim)">
                  {i + 1}.
                </span>
                <input
        ...
        <div className="w-full max-w-md flex flex-col items-center">
          ...
          <img
            src="/melee-qr-bracket-300px.png"
            alt="QR Code"
      ...
      {isAdmin && (
        <div className="w-full max-w-md mt-6 mb-10">
          <EntrantsSection
            entrants={entrants}
            setEntrants={setEntrants}
            isAdmin={isAdmin}
