...
function buildLosersColumns(
  lRounds: string[][],
): { roundIdx: number; subX: number }[][] {
  const cols: { roundIdx: number; subX: number }[][] = [];
  let ri = 0;
  while (ri < lRounds.length) {
    const drop = ri;
    const con =
      ri + 1 < lRounds.length && lRounds[ri + 1].length < lRounds[ri].length
        ? ri + 1
        : null;
    if (con !== null) {
      cols.push([
...
function buildYTable(
  rounds: string[][],
  matches: Record<string, import("@/lib/bracket").Match>,
): number[][] {
...
function SeedBadge({ seed }: { seed: number }) {
  return (
    <span className="text-xs text-(--text-dim) ml-1.5 shrink-0">[{seed}]</span>
  );
}
...
export default function BracketPage() {
  ...
  const handleUndo = (matchId: string) => {
    ...
    const desc =
      `Reset ${match.winner!.name} def. ${match.loser?.name ?? "TBD"}` +
      (affected > 0
        ? ` — also clears ${affected} downstream match${affected > 1 ? "es" : ""}`
        : "");
    setActiveMatchId(null);
  ...
  function lColX(ci: number) {
    return ci * (lColW + COL_GAP);
  }
  ...
  for (let ri = 0; ri < wRounds.length - 1; ri++) {
    ...
    for (let mi = 0; mi < wRounds[ri].length; mi++) {
      ...
      addPath(
        cx + MATCH_W,
        wOffsetY + wYTable[ri][mi] + MATCH_H / 2,
        cx + MATCH_W + COL_GAP / 2,
        wOffsetY + wYTable[ri + 1][pmi] + MATCH_H / 2,
        colX(ri + 1),
        "#2d6b2d",
      );
    }
  ...
  for (let ri = 0; ri < lRounds.length - 1; ri++) {
    ...
    for (let mi = 0; mi < lRounds[ri].length; mi++) {
      ...
      addPath(
        cx + MATCH_W,
        lOffsetY + lYTable[ri][mi] + MATCH_H / 2,
        cx + MATCH_W + (nx - cx - MATCH_W) / 2,
        lOffsetY + lYTable[ri + 1][pmi] + MATCH_H / 2,
        nx,
        "#7a1020",
      );
    }
  ...
  if (wRounds.length > 0) {
    ...
    addPath(
      cx + MATCH_W,
      wOffsetY + wYTable[wRounds.length - 1][0] + MATCH_H / 2,
      cx + MATCH_W + COL_GAP / 2,
      gfY + MATCH_H / 4,
      gfColX,
      "#2d6b2d",
  ...
  if (lRounds.length > 0) {
    ...
    addPath(
      cx + MATCH_W,
      lOffsetY + lYTable[lRounds.length - 1][0] + MATCH_H / 2,
      cx + MATCH_W + COL_GAP / 2,
      gfY + (MATCH_H * 3) / 4,
      gfColX,
      "#7a1020",
    ...
    <main className="h-screen flex flex-col bg-(--bg)">
      ...
      <div className="shrink-0 border-b border-(--border) bg-(--bg-card)">
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          <button
            onClick={() => router.push("/")}
            className="text-sm tracking-widest font-mono text-(--text) hover:text-[#39ff14] transition-colors shrink-0"
          >
            ◀ MENU
          </button>
          <span
          ...
          {isAdmin && renamingName !== null && (
            ...
            >
              <input
                autoFocus
                className="px-2 py-1 text-sm font-mono flex-1 min-w-0"
                value={renamingName}
              ...
              <button
                type="submit"
                className="text-xs px-2 py-1 border border-(--border) text-(--text) hover:border-[#39ff14] transition-colors shrink-0"
              >
                OK
              </button>
              <button
                ...
                className="text-xs px-2 py-1 text-(--text) hover:text-[#e8001c] transition-colors shrink-0"
              >
                ✕
              </button>
            </form>
          ...
          <div className="flex items-center gap-1 shrink-0 ml-auto">
            <button
              ...
              className="w-8 h-8 flex items-center justify-center font-mono text-base text-[var(--text)] border border-[var(--border)] hover:border-[var(--text)] transition-colors"
            >
              −
            </button>
            <button
              onClick={() => setZoom(1)}
              className="px-2 h-8 font-mono text-xs text-[var(--text)] border border-[var(--border)] hover:border-[var(--text)] transition-colors"
            >
              {Math.round(zoom * 100)}%
            </button>
            <button
      ...
      {editMode && moveFrom && (
        <div className="shrink-0 text-center py-1.5 bg-[#f0c00015] border-b border-[#f0c00040] text-xs font-mono text-[#f0c000]">
          ...
          <button className="underline" onClick={() => setMoveFrom(null)}>
            cancel
          </button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto p-4 pt-20">
        <div style={{ width: totalW * zoom, height: (totalH + 36) * zoom }}>
          ...
          >
            <div
              className="absolute text-sm tracking-widest font-bold text-(--text) glow"
              style={{ top: wOffsetY, left: 0, transform: "translateY(-44px)" }}
            >
              ▸ WINNERS BRACKET
            </div>
            <div
              className="absolute text-sm tracking-widest font-bold text-[#e8001c]"
              style={{
                top: lOffsetY,
                left: 0,
                transform: "translateY(-44px)",
                textShadow: "0 0 8px #e8001c",
              }}
            >
              ▸ LOSERS BRACKET
            </div>
            <div
              className="absolute text-xs tracking-widest font-bold text-center text-[#f0c000]"
              style={{
                left: gfColX,
                top: gfY - 18,
                width: MATCH_W,
                textShadow: "0 0 8px #f0c000",
              }}
            >
              ★ GRAND FINALS
            </div>
            ...
            {wRounds.map((round, ri) => {
              ...
              return (
                <div key={`w${ri}`}>
                  <div
                    className="absolute text-xs text-center text-(--text-dim)"
                  ...
                  {round.map((id, mi) => (
                    <div
                      key={id}
                      className="absolute"
                      style={{ left: cx, top: wOffsetY + wYTable[ri][mi] }}
                    >
                      <MatchCard
                        ...
                        winnerColor="#39ff14"
                        borderColor="#2d6b2d"
                        highlight={highlightedMatchIds.has(id)}
                        ready={readyMatchIds.has(id)}
                      />
                    </div>
                  ))}
                </div>
                    ...
                    <div key={`l${roundIdx}`}>
                      ...
                      {round.map((id, mi) => (
                        <div
                          ...
                          style={{
                            left: cx,
                            top: lOffsetY + lYTable[roundIdx][mi],
                          }}
                        >
                          <MatchCard
                            match={state.matches[id]}
                            state={state}
                            onOpen={
                              editMode ? setEditMatchId : setActiveMatchId
                            }
                            editMode={editMode}
                            moveFrom={moveFrom}
                            isAdmin={isAdmin}
              ...
              <div className="absolute" style={{ left: gfColX, top: gfY }}>
                <MatchCard
                  ...
                  onSlotClick={(slot) => {
                    if (!editMode) return;
                    if (!moveFrom) {
                      setMoveFrom({ matchId: gf.id, slot });
                    } else if (
                      moveFrom.matchId === gf.id &&
                      moveFrom.slot === slot
                    ) {
                      setMoveFrom(null);
                    } else {
                  ...
                  winnerColor="#f0c000"
                  borderColor="#6b5500"
                  highlight={highlightedMatchIds.has(gf.id)}
                  ready={readyMatchIds.has(gf.id)}
                />
              </div>
            )}
          </div>
          ...
          return (
            <Suspense>
              <MatchPanel
                ...
                onDQ={handleDQ}
                onRename={(matchId, slot, name) =>
                  update(renamePlayerInMatch(state, matchId, slot, name))
                }
          ...
          <div className="flex items-center gap-0 border-b border-[var(--border)]">
            {(["queue", "standings"] as const).map((tab) => (
              <button
                ...
                className={cn(
                  "px-4 py-2 text-xs tracking-widest transition-colors",
                  drawerTab === tab
              ...
              >
                {tab === "queue"
                  ? `▶ ON DECK (${readyMatches.length})`
                  : `★ STANDINGS`}
              </button>
            ))}
            <button
              onClick={() => setDrawerOpen(false)}
              className="ml-auto px-3 py-2 text-xs text-[var(--text-dim)] hover:text-[var(--text)]"
            >
              ✕
            </button>
          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="text-xs mb-1" style={{ color }}>
                          {m.bracket.toUpperCase().replace("-", " ")} · {m.id}
                        </div>
              ...
              <div className="flex flex-col gap-1 px-4 py-3 min-w-50">
                {standings.length === 0 ? (
                  ...
                  standings.map((s, i) => (
                    ...
                    >
                      <span className="text-(--text-dim) w-12 shrink-0">
                        {s.place}
                      </span>
      ...
      {confirmUndo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-(--bg-card) border border-(--border) p-6 max-w-sm w-full mx-4 font-mono">
            <div className="text-base tracking-widest font-bold text-(--text) mb-3">
              ⚠ UNDO RESULT?
            </div>
            <div className="text-sm text-(--text-dim) mb-6">
              {confirmUndo.description}
            </div>
      ...
      {editMatchId &&
        ...
        (() => {
          ...
          const p1 =
            m.p1Source && typeof m.p1Source === "object" ? m.p1Source : null;
          const p2 =
            m.p2Source && typeof m.p2Source === "object" ? m.p2Source : null;
          return (
            <div
            ...
            >
              <div
                className="bg-[var(--bg-card)] border border-[#f0c000] p-5 w-full max-w-sm mx-4 mb-4 sm:mb-0 font-mono"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-sm tracking-widest text-[#f0c000]">
                    ✎ RENAME PLAYERS
                  </span>
                  <button
                    onClick={() => setEditMatchId(null)}
                ...
                <div className="mt-3 text-xs text-[var(--text-dim)] text-center">
                  To move players between matches, click slots directly on the
                  bracket.
                </div>
              </div>
            </div>
    ...
    <div className="flex items-center gap-2 py-2 border-b border-[var(--border)]">
      ...
      {editing ? (
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            onRename(val.trim() || player.name);
            setEditing(false);
          }}
        >
          <input
            autoFocus
            className="flex-1 px-2 py-0.5 text-sm font-mono"
            value={val}
          ...
          <button
            type="submit"
            className="text-xs px-2 border border-(--border) text-(--text) hover:border-[#39ff14]"
          >
            OK
          </button>
        </form>
        ...
        <>
          ...
          <button
            ...
            className="text-xs text-[var(--text-dim)] hover:text-[#f0c000] transition-colors"
          >
            rename
          </button>
        </>
...
function MatchCard({
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-(--bg-card) cursor-pointer",
        ready && !highlight && !editMode && "ready-pulse",
      )}
      ...
      style={{
        ...
        boxShadow: highlight
          ...
          : editMode
            ? "0 0 8px #f0c00055"
            : ready
              ? `0 0 8px ${winnerColor}55`
              : hasResult
                ? `0 0 10px ${winnerColor}33`
                : "none",
      }}
    >
      {(["p1", "p2"] as const).map((slot, i) => {
        ...
        const isTarget =
          editMode &&
          moveFrom &&
          !(moveFrom.matchId === match.id && moveFrom.slot === slot);
        return (
          <div
            key={slot}
            className={cn(
            ...
            style={{
              ...
              borderBottom:
                i === 0
                  ? `1px solid ${editMode ? "#f0c00040" : borderColor}`
                  : "none",
            ...
            <span className="flex items-center overflow-hidden min-w-0 flex-1">
              <span className="overflow-hidden text-ellipsis whitespace-nowrap text-xs min-w-0">
                {player ? (
                  ...
                  <span className="text-(--text-dim) italic">TBD</span>
                )}
              </span>
              {isAdmin && player?.seed && <SeedBadge seed={player.seed} />}
            </span>
            {editMode ? (
              <span className="text-xs shrink-0 text-[#f0c00080]">
                {isSelected ? "✦ MOVING" : "↕"}
              </span>
            ) : gameScore !== null ? (
              <span
                className="text-xs shrink-0"
                style={{ color: isWinner ? winnerColor : "var(--text-dim)" }}
              >
                {gameScore}
              </span>
            ) : (
